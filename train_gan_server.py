...
class CustomDataParallel(nn.DataParallel):
    ...
    def gather(self, outputs, output_device):
        ...
        if cfg.pred_seg:
            ...
            seg_list  = []
            pred_list = []
            for i, (loss, seg, pred) in enumerate(outputs):
                seg_list  += [j.to(output_device) for j in seg]
                pred_list += [j.to(output_device) for j in pred]
                loss_list.append(loss)

            for k in loss_list[0]:
                losses[k] = torch.stack([loss[k].to(output_device) for loss in loss_list])
...
def seg_mask_clas(seg_list, pred_list, datum):
    ...
    target_list = [target.to(cuda0) for target in datum[1][0]]
